...
const { getJsonConfig, updateJsonConfig, getJsonFilePath } = require('../utils/json-config-helper');
...
async function deploy({ projPath, projectId, domain, stage, secret }) {
  try {
    ...
    const deployPath = path.join(projPath, jsonConfig.outDir);
    ...
    const options = {
      files: {
        include: [...deployList.map(f => f.key), getJsonFilePath(projPath)],
      },
    };
