...
function getJsonFilePath(projPath) {
  return path.join(projPath, config.JSON_CONFIG_FILE || 'app.json');
}
...
function readJson(projPath) {
  try {
    return JSON.parse(fs.readFileSync(path.join(projPath, config.JSON_CONFIG_FILE || 'app.json')));
  } catch (error) {
    return {};
  }
}
...
function writeJson(projPath, json) {
  ...
  try {
    // write ordered
    fs.writeFileSync(
      path.join(projPath, config.JSON_CONFIG_FILE || 'app.json'),
      JSON.stringify(
        {
          projectId,
          outDir: outDir || '.',
